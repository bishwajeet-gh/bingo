class JsonBinService {
    constructor() {
        ...
        this.mockProgress = {};
        this.mockSettings = { boardSize: 5 }; // Default board size for mock
    ...
    async getGameSettings() {
        ...
        try {
            const response = await this.fetchWithRetry(`${this.baseUrl}/${CONFIG.JSONBIN.SETTINGS_BIN_ID}/latest`, {
                method: 'GET'
            });
            const data = await response.json();
            return data || { boardSize: CONFIG.BINGO.BOARD_SIZE };
        } catch (error) {
            console.error('Error fetching game settings:', error);
            return { boardSize: CONFIG.BINGO.BOARD_SIZE };
        }
    }
    ...
    async updateGameSettings(settings) {
        ...
        try {
            const response = await this.fetchWithRetry(`${this.baseUrl}/${CONFIG.JSONBIN.SETTINGS_BIN_ID}`, {
                method: 'PUT',
    ...
    async validateUser(username) {
        try {
            ...
            const users = Array.isArray(gameData.users) ? gameData.users : [];
            ...
            return {
                isValid: userExists,
                user: userExists ? { username: username } : null
            };
        } catch (error) {
            ...
            return {
                isValid: false,
                user: null,
                error: 'Unable to validate user. Please check your connection and try again.'
            };
        }
    }
}
